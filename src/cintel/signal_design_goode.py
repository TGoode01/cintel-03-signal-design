...
def main() -> None:
    ...
    # ----------------------------------------------------
    # STEP 2.3B: DEFINE THE SUCCESS RATE SIGNAL RECIPE
    # ----------------------------------------------------
    # Success rate = (requests - errors) / requests
    calculated_success_rate: pl.Expr = (pl.col("requests") - pl.col("errors")) / pl.col(
        "requests"
    )
